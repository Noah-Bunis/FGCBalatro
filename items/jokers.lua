...
SMODS.Joker {
    ...
    loc_txt = {
        ...
        ['text'] = {
            ...
            [3] = "and add a {C:attention}random Joker{}",
            [4] = "of the same rarity to hand."
        }
    },
    pos = {x=0,y=0},
    cost = 5,
            ...
            if my_pos and G.jokers.cards[my_pos + 1] and not SMODS.is_eternal(G.jokers.cards[my_pos + 1], card) and not G.jokers.cards[my_pos + 1].getting_sliced then
                ...
                G.E_MANAGER:add_event(Event({
                    func = function()
                        ...
                        SMODS.add_card {
                                        set = 'Joker',
                                        rarity = ({'Common', 'Uncommon', 'Rare', 'Legendary'})[sliced_card.config.center.rarity] or sliced_card.config.center.rarity,
                                    }
                        return true
                    end
                }))
                return {
                    message = "Random Select!",
                    colour = G.C.RED,
                    no_juice = true
                }
            end
